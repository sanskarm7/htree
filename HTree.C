...
HTree::HTree() {
  Node *root = new DataNode(); // the first node is the root as well as leaf
                               // (i.e. a data node)
  root->level = 0;
  HTreeRoot = root;
  height = 1;
...
// bilkload
// each line of the file must contain one record, NUMDIMS float pt numbers
// separated by spaces
...
HTree::HTree(char *dumpfile) {
  ...
  firstitem = 0; // since data is loaded from a file, a new insert won't be the
                 // first item
  fclose(fp);
  numemptynodes = 0;
  ...
  objectcount = 0;
  if (ELSWANTED > 0)
    HTreeCreateELSs(HTreeRoot, &rootRect, &objectcount);
  ...
  if (n->level > 0) // this is an internal node in the tree
  {
    if (n->type == Node::MONODE) {
      ...
      for (i = 0; i < N->numInternalNodes; i++) {
        fread(&(N->internal[i]), sizeof(struct OverlappingkdTreeInternalNode),
              1, fp);
...
int HTree::Dump(Node *n, FILE *fp) {
  ...
  if (n->level > 0) // this is an internal node in the tree
  {
    if (n->type == Node::MONODE) {
      ...
      for (i = 0; i < N->numInternalNodes; i++) {
        fwrite(&(N->internal[i]), sizeof(struct OverlappingkdTreeInternalNode),
               1, fp);
      }
  ...
  } else // this is a leaf node
  {
    ...
    for (i = 0; i < N->count; i++) {
      ...
      // printf("%f %f\n", N->branch[i].point.position[0],
      // N->branch[i].point.position[NUMDIMS-1]);
    }
...
int HTree::HTreeInsert2(Point *r, int tid, Node *n, Node **new_node,
                        struct split **newsplit, int level,
                        Rect *parentNodeRect, int cascading_splits_allowed,
  ...
  if (n->level > level) {
    if (n->type == Node::MONODE) // minimum overlap node
    {
      ...
      if (!HTreeInsert2(r, tid, N->leaf[i].child, &n2, &lastsplit, level,
                        &childNodeRect, cascading_splits_allowed, reinsertList,
                        numItems)) {
      ...
      {
        ret_val = N->AddChild(n2, (MONode **)new_node, i, lastsplit, newsplit,
                              parentNodeRect);
        if (*new_node != NULL)
          assert((((MONode *)*new_node)->numLeafNodes < OFNODECARD) &&
                 (((MONode *)*new_node)->numLeafNodes > 0));
        free(lastsplit);
        return ret_val;
      }
    ...
    else //  n->type == OVERLAP_FREE
    {
      ...
      // invalidate the els
      N->leaf[i].elsvalid = 0;
      if (!HTreeInsert2(r, tid, N->leaf[i].child, &n2, &lastsplit, level, NULL,
                        cascading_splits_allowed, reinsertList, numItems)) {
        // data node was not split
        // nothing needs to be done
        // if i was following an eager policy, i would have recomputed the new
        // els here
        return 0;
      } else // data node was split and n2 is the new data node
      {
        ret_val = N->AddChild(n2, (OFNode **)new_node, i, lastsplit, newsplit,
                              parentNodeRect, cascading_splits_allowed,
                              reinsertList, numItems);
        if (*new_node != NULL)
          assert((((OFNode *)*new_node)->numLeafNodes < OFNODECARD) &&
                 (((OFNode *)*new_node)->numLeafNodes > 0));
        free(lastsplit);
        return ret_val;
      }
    }
...
int HTree::Insert(Point *R, int Tid) {
  if (firstitem) {
    ...
    rootRect = rootRect.CombineRect(R);
  ...
  if (num_reinsertions > 0) {
    ...
    for (int i = 0; i < num_reinsertions; i++)
      HTreeInsert(&(reinsertion_list[i].point), reinsertion_list[i].tid,
                  &HTreeRoot, 0);
  }
  return 0;
}
...
//
int HTree::HTreeInsert(Point *R, int Tid, Node **Root, int Level,
                       int cascading_splits_allowed, ReinsertItem *reinsertList,
                       int *numItems) {
  Point *r = R;
  int tid = Tid;
  Node **root = Root;
  ...
  if (HTreeInsert2(r, tid, *root, &newnode, &newSplit, level, &rootRect,
                   cascading_splits_allowed, reinsertList,
                   numItems)) /* root split */
  {
    if ((*root)->level ==
        0) // current root is a data node but new root would be a OFNode
    {
    ...
    else if ((*root)->level == 1) // current root is a OFNode but new root would
                                  // be an ordinary node
    {
...
Rect HTree::HTreeCreateELSs(Node *n, Rect *indexedRegion,
                            unsigned long *objectcount) {
  int i, ii;
  Rect livespace;
  Rect newlivespace;
  ...
  *objectcount = 0;
  if (n->level > 0) // this is an internal node in the tree
  {
    if (n->type == Node::MONODE) // this is an MO node
    {
      ...
      for (i = 0; i < N->numLeafNodes; i++) {
        ...
        livespace =
            HTreeCreateELSs(childNode, &MOchildRectlist[i], &totalcount);
        ...
        *objectcount += totalcount;
        ...
        if (i == 0)
          ...
          totallivespace = totallivespace.CombineRect(&livespace);
      }
    ...
    } else // this is an OFnode (type is OVERLAP_FREE)
    {
      ...
      for (i = 0; i < N->numLeafNodes; i++) {
        ...
        // N->leaf[i].ELS=(unsigned int *)malloc(ELSARRAYLEN*sizeof(unsigned
        // int));
  ...
  // level==0 leaf nodes should not reach here
  assert(false);
  return Rect();
}
...
int HTree::IsPageFault(Node *node) {
  ...
  // we are using the memory addresses of the nodes to identify them, as their
  // id this will not work when the nodes are disk-based; but then we won't need
  // a buffer simulator
  if (NUM_PAGES_IN_MEMORY <= 0)
    return 1;
  ...
  {
    if (pages_in_memory.size() <
        NUM_PAGES_IN_MEMORY) // can just bring it in memory, no one needs to be
                             // thrown out
    {
      // nanosleep(&io_delay, NULL);   // do the disk access
      pages_in_memory.push_back((long)node); // bring in the page
    } else // some one has to be thrown out, so the oldest guy is chosen (first
           // guy in list)
    {
...
void HTree::ClearBuffer() {
  while (!(pages_in_memory.empty()))
    pages_in_memory.pop_front();
}
